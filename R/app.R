 ...
 # logo image
  ...
  output$info <- renderUI({
    tagList(a("Github page", href="https://github.com/LeoEgidi"),
           ...
           "In order to reduce the complexity of the package and the Shiny app, the code is intended only for the computation of congeneric latent constructs. CLC does not include additional functions that are widely available in the most commonly used statistical packages like exploratory factor analysis, principal component analysis, or confirmatory factor analysis.",
            div(style = "padding: 5px 5px"),
            div(HTML("<em>At what point in the data analysis process CLC estimator should be used?</em>")),
            div(style = "padding: 2px 2px"),
            "CLC estimator should be integrated in the data analysis process after having checked the items (including reliability analysis) to be retained in the latent constructs.",
            div(style = "padding: 5px 5px"),
            div(HTML("<em>How to deal with missing data, outliers, and other data issue in CLC estimator?</em>")),
            div(style = "padding: 2px 2px"),
            "In order to keep the code and the app as simple as possible, CLS estimator does not make any type of assumption or calculation for missing data, outliers, or incomplete data. The users are required to deal with these issues before moving the dataset inside CLC estimator. Specifically for missing data, CLC estimator has the option to skip from calculation constructs with missing data (option “ignore missing data” selected) or to provide a warning message to users in the case missing data are identified (option “ignore missing data” not selected).",
            div(style = "padding: 5px 5px"),
            div(HTML("<em>How to create a usable .csv file for CLC estimator?</em>")),
            div(style = "padding: 2px 2px"),
            div(HTML("CLC estimator reads standard generated .csv outputs from the most common statistical packages. The first row of the .csv must contain the variables’ labels (in R terms, header=T). The .csv files should be in UNICODE. As sample dataset is available on the GitHub page of the app to allow users understanding the proper format of the .csv file (SampleDataset.csv)  to be used with CLC estimator.
             <b> ATTENTION According to the current verion of the CLC estimator, a .csv file may be stored only locally, which means that to be able to save your .csv file at the end of your session you'll need to launch the CLC estimator from your RStudio consolle and type the following lines of code as follows:</b>")),
            div(style = "padding: 5px 5px"),
            "> library(devtools)  # required package",
            div(style = "padding: 1px 1px"),
            "> install_github(leoegidi/clc) # install clc package from Github directly",
            div(style = "padding: 1px 1px"),
           "> library('clc')",
            div(style = "padding: 1px 1px"),
           "> clc()",
            div(style = "padding: 5px 5px"),
            div(HTML( "<em>What type of factor extraction method should be used?</em>")),
            div(style = "padding: 2px 2px"),
            "If social scientists are adopting previously validated scales, the same extraction methods of the given scale should be used. If the extraction method is not specified by the creators of the scales or the scale is not previously validated, maximum likelihood is suggested. However, for a  comprehensive overview and additional guidance on this topic see Thompson (2004).",
            div(style = "padding: 5px 5px"),
            div(HTML("<em>What type of output CLC estimator produces?</em>")),
            div(style = "padding: 2px 2px"),
            "CLC estimator generates a standard .csv file with variable names in the first row and values in the other rows. The generated .csv output can then be easily imported to commonly used statistical packages.",
            div(style = "padding: 6px 6px"),
            div(HTML("<b>References</b>")),
            div(style = "padding: 2px 2px"),
            div(HTML("Rasoolimanesh, S. M., Ringle, C. M., Sarstedt, M., & Olya, H. (2021). The combined use of symmetric and asymmetric approaches: Partial least squares-structural equation modeling and fuzzy-set qualitative comparative analysis. <em>International Journal of Contemporary Hospitality Management</em>, 33(5), 1571-1592. https://doi.org/10.1108/IJCHM-10-2020-1164")),
            div(style = "padding: 2px 2px"),
            div(HTML("Thompson, B. (2004). <em>Exploratory and confirmatory factor analysis: Understanding concepts and applications</em>. American Psychological Association. https://doi.org/10.1037/10694-000")),
            div(style = "padding: 5px 5px"),
